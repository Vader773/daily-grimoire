...
export const FullScreenStats = ({ isOpen, onClose, onNavigateToStreak, league: leagueProp, onOpenLeagueHall }: FullScreenStatsProps) => {
  ...
  const timeStats = useMemo(() => {
    ...
    return {
      ...
      week: weekTasks.length + (stats.taskHistory?.filter(h => new Date(h.date) >= startOfWeek && h.date !== today).reduce((acc, curr) => acc + curr.count, 0) || 0),
      month: monthTasks.length + (stats.taskHistory?.filter(h => new Date(h.date) >= startOfMonth && h.date !== today).reduce((acc, curr) => acc + curr.count, 0) || 0),
      year: yearTasks.length + (stats.taskHistory?.filter(h => new Date(h.date) >= startOfYear && h.date !== today).reduce((acc, curr) => acc + curr.count, 0) || 0),
      total: completedTasks.length + (stats.taskHistory?.reduce((acc, curr) => acc + curr.count, 0) || 0),
      weeklyHabits: completedTasks.filter(t => t.isHabitTask && new Date(t.completedAt!) >= startOfWeek).length,
    };
  }, [tasks, stats.taskHistory]); // Add taskHistory dependency
                    ...
                    <div className="p-4 rounded-xl border border-dashed border-muted-foreground/30 bg-muted/10">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="flex gap-2 justify-center">
                          ...
                          >
                            Simulate Next Day ({getTodayDate()})
                          </Button>
                        </div>
